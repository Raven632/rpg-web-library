...
![Скриншот интерфейса — обложки скрыты встроенной кнопкой](/rest/img/Example.png)
...
2. (Необязательно) Создайте файл настроек:
Без этого файла сервер тоже запустится на значениях по умолчанию — он нужен, когда хочешь свой SESSION_SECRET или ключ ScraperAPI.
   ```bash
   cp .env.example .env
   ```
